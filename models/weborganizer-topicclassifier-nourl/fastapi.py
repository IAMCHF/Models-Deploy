...
# -*- coding: utf-8 -*-
"""
FastAPI 服务 - WebOrganizer/TopicClassifier-NoURL
任务: 网页内容主题分类(24类)
模态: text

模型加载与推理代码已根据 HuggingFace 模型页面官方示例适配：
  - 镜像站优先: https://hf-mirror.com/WebOrganizer/TopicClassifier-NoURL
  - 官方兜底:   https://huggingface.co/WebOrganizer/TopicClassifier-NoURL
  - 参考页面 "Usage" 代码片段(需 trust_remote_code=True)
"""
...
logger = logging.getLogger("weborganizer-topicclassifier-nourl")

WEIGHTS_DIR = Path(__file__).resolve().parent / "weights"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

app = FastAPI(title="weborganizer-topicclassifier-nourl", version="1.0.0")
...
async def predict(req: PredictRequest):
    """
    预测接口
    - 输入: req.data (base64 编码的 text 数据，网页正文)
    ...
    try:
        ...
        with torch.no_grad():
            outputs = model(**inputs)
            probs = outputs.logits.softmax(dim=-1)
        predicted_class_id = int(probs.argmax(dim=-1).item())
