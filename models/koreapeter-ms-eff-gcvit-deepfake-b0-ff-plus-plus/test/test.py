...
# -*- coding: utf-8 -*-
"""
测试脚本 - koreapeter-ms-eff-gcvit-deepfake-b0-ff-plus-plus
- /health 断言 status=ok
- /predict 契约: 输入=人脸JPG(base64, 两张), 输出=JSON [{label, score}]
...
logger = logging.getLogger("test_koreapeter-ms-eff-gcvit-deepfake-b0-ff-plus-plus")
...
def build_payload():
    ...
    data = load_bytes("sample_female.jpg")
    logger.info("使用测试数据: sample_female.jpg (%d bytes)", len(data))
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: koreapeter-ms-eff-gcvit-deepfake-b0-ff-plus-plus @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
