...
# -*- coding: utf-8 -*-
"""
测试脚本 - skywork-skywork-reward-v2-qwen3-0-6b
...
logger = logging.getLogger("test_skywork-skywork-reward-v2-qwen3-0-6b")
...
def get_test_data():
    """获取测试数据（base64 编码）"""
    # 查找 test_data 目录中的样例文件
    sample_files = list(TEST_DATA_DIR.glob("*"))
    ...
    if sample_files:
        ...
        # 根据模态生成随机数据
        logger.warning("无测试数据文件，根据模态(%s)生成随机数据", "text")
        if "text" == "text":
            raw = "这是一条测试文本。This is a test text.".encode("utf-8")
        elif "text" in ("image", "video"):
            import random
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: skywork-skywork-reward-v2-qwen3-0-6b @ %s", BASE_URL)
    logger.info("=" * 50)
