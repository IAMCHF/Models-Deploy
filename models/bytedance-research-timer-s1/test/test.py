...
# -*- coding: utf-8 -*-
"""
测试脚本 - bytedance-research-timer-s1
- /health 断言 status=ok
- /predict 契约: 输入=JSON {seqs: [[..]], forecast_length}(base64), 输出=JSON {quantiles}
...
logger = logging.getLogger("test_bytedance-research-timer-s1")
...
def build_payload():
    ...
    inner = json.dumps({"seqs": [vals], "forecast_length": 32})
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: bytedance-research-timer-s1 @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
