...
# -*- coding: utf-8 -*-
"""
测试脚本 - ibm-granite-granite-timeseries-patchtst-fm-r1
- /health 断言 status=ok
- /predict 契约: 输入=JSON {target_values: [..], prediction_length}(base64), 输出=JSON {forecast}
...
logger = logging.getLogger("test_ibm-granite-granite-timeseries-patchtst-fm-r1")
...
def build_payload():
    ...
    inner = json.dumps({"target_values": vals, "prediction_length": 24})
...
def show_result(decoded: bytes):
    ...
    main = payload.get("forecast")
    assert main, f"返回缺少 forecast: {str(payload)[:200]}"
    logger.info("PatchTST 分位数预测, 共 %d 项:", len(main))
    for item in main[:3]:
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: ibm-granite-granite-timeseries-patchtst-fm-r1 @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
