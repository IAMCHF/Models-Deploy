...
# -*- coding: utf-8 -*-
"""
测试脚本 - paddlepaddle-pp-chart2table
- /health 断言 status=ok
- /predict 契约: 输入=图表PNG(base64), 输出=图表转表格文本
...
logger = logging.getLogger("test_paddlepaddle-pp-chart2table")
...
def show_result(decoded: bytes):
    ...
    logger.info("图表转表格结果(共 %d 字符), 已保存 result.txt, 预览:", len(text))
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: paddlepaddle-pp-chart2table @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
