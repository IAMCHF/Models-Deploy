...
# -*- coding: utf-8 -*-
"""
测试脚本 - prior-labs-tabpfn-v2-reg
- /health 断言 status=ok
- /predict 契约: 输入=JSON {X_train, y_train, X_test}(base64 文件原文), 输出=JSON {predictions}
...
logger = logging.getLogger("test_prior-labs-tabpfn-v2-reg")
...
def build_payload():
    ...
    data = load_bytes("sample.json")
    logger.info("使用测试数据: sample.json (%d bytes)", len(data))
...
def show_result(decoded: bytes):
    ...
    main = payload.get("predictions")
    assert main, f"返回缺少 predictions: {str(payload)[:200]}"
    logger.info("TabPFN-v2 回归预测, 共 %d 项:", len(main))
    logger.info("  前6项: %s", 6, main[:6])
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: prior-labs-tabpfn-v2-reg @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
