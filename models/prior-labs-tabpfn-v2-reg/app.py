...
# -*- coding: utf-8 -*-
"""
FastAPI 服务 - Prior-Labs/TabPFN-v2-reg
任务: 表格回归(Nature论文,零样本)
模态: tabular

[重要] 模型加载代码需从 HuggingFace 模型页面获取真实部署代码后填入下方 TODO 区域。
  - 镜像站优先: https://hf-mirror.com/Prior-Labs/TabPFN-v2-reg
  - 官方兜底:   https://huggingface.co/Prior-Labs/TabPFN-v2-reg
...
logger = logging.getLogger("prior-labs-tabpfn-v2-reg")

WEIGHTS_DIR = Path(__file__).resolve().parent / "weights"

app = FastAPI(title="prior-labs-tabpfn-v2-reg", version="1.0.0")

# ============================================================
# 模型加载区域 — 从 HuggingFace 模型页面及官方文档获取的真实部署代码
# 来源: https://hf-mirror.com/Prior-Labs/TabPFN-v2-reg
...
def load_model():
    ...
    global model
    if model is None:
        logger.info("正在加载 TabPFN v2 回归模型，权重目录: %s", WEIGHTS_DIR)
        # 查找 weights 目录下的 .ckpt 文件
        ckpt_files = glob.glob(str(WEIGHTS_DIR / "*.ckpt"))
        if ckpt_files:
            model = TabPFNRegressor(model_path=ckpt_files[0])
            logger.info("TabPFN v2 回归模型加载完成，使用本地权重: %s", ckpt_files[0])
        else:
            # 未找到本地权重文件，使用默认（从 HF 下载）
            model = TabPFNRegressor()
            logger.info("TabPFN v2 回归模型初始化完成（将自动从 HF 下载权重）")
...
async def predict(req: PredictRequest):
    ...
    try:
        ...
        result_data = {"predictions": predictions.tolist()}
