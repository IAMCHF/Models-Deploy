...
- /health 断言 status=ok
- /predict 契约: 输入=JSON {past_values: [[..]], forecast_context_len}(base64), 输出=JSON {mean_predictions}
...
def build_payload():
    ...
    inner = json.dumps({"past_values": [vals], "forecast_context_len": 512})
...
def show_result(decoded: bytes):
    ...
    main = payload.get("mean_predictions")
