...
# -*- coding: utf-8 -*-
"""
测试脚本 - paddlepaddle-pp-doclayout-plus-l
- /health 断言 status=ok
- /predict 契约: 输入=文档页PNG(base64), 输出=JSON 布局区域列表
...
logger = logging.getLogger("test_paddlepaddle-pp-doclayout-plus-l")
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: paddlepaddle-pp-doclayout-plus-l @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
