...
# -*- coding: utf-8 -*-
"""
FastAPI 服务 - PaddlePaddle/PP-DocLayout_plus-L
任务: 高精度文档布局区域定位(20类)
模态: image

[重要] 模型加载代码需从 HuggingFace 模型页面获取真实部署代码后填入下方 TODO 区域。
  - 镜像站优先: https://hf-mirror.com/PaddlePaddle/PP-DocLayout_plus-L
  - 官方兜底:   https://huggingface.co/PaddlePaddle/PP-DocLayout_plus-L
...
logger = logging.getLogger("paddlepaddle-pp-doclayout-plus-l")

WEIGHTS_DIR = Path(__file__).resolve().parent / "weights"

app = FastAPI(title="paddlepaddle-pp-doclayout-plus-l", version="1.0.0")

# ============================================================
# 模型加载区域 — 基于 PaddlePaddle/PP-DocLayout_plus-L 官方示例代码适配
# 使用 paddleocr.LayoutDetection (PaddlePaddle 引擎)
# 源码参考: https://huggingface.co/PaddlePaddle/PP-DocLayout_plus-L
# ============================================================
_model = None


def load_model():
    """加载 PP-DocLayout_plus-L 高精度文档布局检测模型"""
    global _model
    if _model is not None:
        return
    from paddleocr import LayoutDetection
    logger.info("正在加载 PP-DocLayout_plus-L 模型...")
    _has_local_weights = (
        WEIGHTS_DIR.exists()
    ...
    if _has_local_weights:
        ...
        _model = LayoutDetection(model_name="PP-DocLayout_plus-L")
    logger.info("PP-DocLayout_plus-L 模型加载完成")
...
async def predict(req: PredictRequest):
    ...
    # 输入: base64 编码的图像 → 高精度文档布局区域定位 (20类)
