...
# -*- coding: utf-8 -*-
"""
测试脚本 - autogluon-chronos-2
- /health 断言 status=ok
- /predict 契约: 输入=JSON {context: CSV(id,timestamp,target), prediction_length}(base64), 输出=预测CSV
...
logger = logging.getLogger("test_autogluon-chronos-2")
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: autogluon-chronos-2 @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
