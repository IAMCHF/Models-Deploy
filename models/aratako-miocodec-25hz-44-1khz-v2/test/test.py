...
# -*- coding: utf-8 -*-
"""
测试脚本 - aratako-miocodec-25hz-44-1khz-v2
- /health 断言 status=ok
- /predict 契约: 输入=WAV(base64), 输出=重建WAV
...
logger = logging.getLogger("test_aratako-miocodec-25hz-44-1khz-v2")
...
def build_payload():
    ...
    data = load_bytes("sample.wav")
    logger.info("使用测试数据: sample.wav (%d bytes)", len(data))
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: aratako-miocodec-25hz-44-1khz-v2 @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
