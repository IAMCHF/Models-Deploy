...
# -*- coding: utf-8 -*-
"""
测试脚本 - ibm-research-ttm-r3
- /health 断言 status=ok
- /predict 契约: 输入=JSON {past_values: [[..]]}(base64), 输出=JSON {predictions, shape}
...
logger = logging.getLogger("test_ibm-research-ttm-r3")
...
def build_payload():
    ...
    inner = json.dumps({"past_values": [vals]})
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: ibm-research-ttm-r3 @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
