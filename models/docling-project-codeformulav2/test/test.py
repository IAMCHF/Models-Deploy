...
# -*- coding: utf-8 -*-
"""
测试脚本 - docling-project-codeformulav2
- /health 断言 status=ok
- /predict 契约: 输入=公式/代码截图PNG(base64), 输出=识别文本([Code]/[Formula])
...
logger = logging.getLogger("test_docling-project-codeformulav2")
...
def build_payload():
    ...
    data = load_bytes("sample.png")
    logger.info("使用测试数据: sample.png (%d bytes)", len(data))
...
def show_result(decoded: bytes):
    ...
    logger.info("公式/代码识别结果(共 %d 字符), 已保存 result.txt, 预览:", len(text))
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: docling-project-codeformulav2 @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
