...
# -*- coding: utf-8 -*-
"""
测试脚本 - k-iwa-time-anchor-modernbert-32m
- /health 断言 status=ok
- /predict 契约: 输入=JSON {target_context: [..], prediction_length, quantile_levels}(base64), 输出=JSON {forecast_rows}
...
logger = logging.getLogger("test_k-iwa-time-anchor-modernbert-32m")
...
def build_payload():
    ...
    inner = json.dumps({"target_context": vals, "prediction_length": 24, "quantile_levels": [0.1, 0.5, 0.9]})
...
def show_result(decoded: bytes):
    ...
    main = payload.get("forecast_rows")
    assert main, f"返回缺少 forecast_rows: {str(payload)[:200]}"
    logger.info("TimeAnchor 预测, 共 %d 项:", len(main))
    for item in main[:3]:
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: k-iwa-time-anchor-modernbert-32m @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
