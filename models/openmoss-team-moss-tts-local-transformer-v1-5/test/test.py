...
# -*- coding: utf-8 -*-
"""
测试脚本 - openmoss-team-moss-tts-local-transformer-v1-5
- /health 断言 status=ok
- /predict 契约: 输入=中文文本(base64), 输出=合成语音WAV
...
logger = logging.getLogger("test_openmoss-team-moss-tts-local-transformer-v1-5")
...
def build_payload():
    ...
    data = load_bytes("sample.txt")
    logger.info("使用测试数据: sample.txt (%d bytes)", len(data))
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: openmoss-team-moss-tts-local-transformer-v1-5 @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
