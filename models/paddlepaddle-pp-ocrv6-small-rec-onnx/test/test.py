...
# -*- coding: utf-8 -*-
"""
测试脚本 - paddlepaddle-pp-ocrv6-small-rec-onnx
- /health 断言 status=ok
- /predict 契约: 输入=文字行PNG(base64), 输出=JSON 识别文本列表
...
logger = logging.getLogger("test_paddlepaddle-pp-ocrv6-small-rec-onnx")
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: paddlepaddle-pp-ocrv6-small-rec-onnx @ %s", BASE_URL)
    logger.info("=" * 50)
    try:
        test_health()
