...
# -*- coding: utf-8 -*-
"""
FastAPI 服务 - PaddlePaddle/PP-OCRv6_small_rec_onnx
任务: OCR文本识别(50种语言)
模态: image

[重要] 模型加载代码需从 HuggingFace 模型页面获取真实部署代码后填入下方 TODO 区域。
  - 镜像站优先: https://hf-mirror.com/PaddlePaddle/PP-OCRv6_small_rec_onnx
  - 官方兜底:   https://huggingface.co/PaddlePaddle/PP-OCRv6_small_rec_onnx
...
logger = logging.getLogger("paddlepaddle-pp-ocrv6-small-rec-onnx")
...
def load_model():
    ...
    if _has_local_weights:
        _model = TextRecognition(model_name="PP-OCRv6_small_rec", model_dir=str(WEIGHTS_DIR), engine="onnxruntime")
    else:
...
async def predict(req: PredictRequest):
    ...
    try:
        ...
        # 收集结果: 只提取识别文本与置信度, 避免整图像素数组
        results = []
        for res in output:
            try:
                item = {}
                for k in ("rec_text", "rec_score"):
                    try:
                        item[k] = res[k]
                    except Exception:
                        pass
