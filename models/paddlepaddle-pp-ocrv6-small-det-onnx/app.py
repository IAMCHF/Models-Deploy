...
# -*- coding: utf-8 -*-
"""
FastAPI 服务 - PaddlePaddle/PP-OCRv6_small_det_onnx
任务: OCR文本检测(轻量)
模态: image

[重要] 模型加载代码需从 HuggingFace 模型页面获取真实部署代码后填入下方 TODO 区域。
  - 镜像站优先: https://hf-mirror.com/PaddlePaddle/PP-OCRv6_small_det_onnx
  - 官方兜底:   https://huggingface.co/PaddlePaddle/PP-OCRv6_small_det_onnx
...
logger = logging.getLogger("paddlepaddle-pp-ocrv6-small-det-onnx")

WEIGHTS_DIR = Path(__file__).resolve().parent / "weights"

app = FastAPI(title="paddlepaddle-pp-ocrv6-small-det-onnx", version="1.0.0")

# ============================================================
# 模型加载区域 — 基于 PaddlePaddle/PP-OCRv6_small_det_onnx 官方示例代码适配
# 使用 paddleocr.TextDetection + onnxruntime 引擎
# 源码参考: https://huggingface.co/PaddlePaddle/PP-OCRv6_small_det_onnx
# ============================================================
_model = None


def load_model():
    """加载 PP-OCRv6_small_det 文本检测模型 (ONNX)"""
    global _model
    if _model is not None:
        return
    from paddleocr import TextDetection
    logger.info("正在加载 PP-OCRv6_small_det 模型 (ONNX)...")
    _has_local_weights = (
        WEIGHTS_DIR.exists()
        and any(f for f in WEIGHTS_DIR.iterdir() if f.name != ".gitkeep")
    )
    if _has_local_weights:
        _model = TextDetection(model_name="PP-OCRv6_small_det", model_dir=str(WEIGHTS_DIR), engine="onnxruntime")
    else:
        _model = TextDetection(model_name="PP-OCRv6_small_det", engine="onnxruntime")
    logger.info("PP-OCRv6_small_det 模型加载完成")
