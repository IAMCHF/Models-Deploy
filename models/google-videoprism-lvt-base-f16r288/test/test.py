...
# -*- coding: utf-8 -*-
"""
测试脚本 - google-videoprism-lvt-base-f16r288
...
logger = logging.getLogger("test_google-videoprism-lvt-base-f16r288")
...
def get_test_data():
    ...
    if sample_files:
        ...
        # 根据模态生成随机数据
        logger.warning("无测试数据文件，根据模态(%s)生成随机数据", "video")
        if "video" == "text":
            raw = "这是一条测试文本。This is a test text.".encode("utf-8")
        elif "video" in ("image", "video"):
            import random
...
def main():
    logger.info("=" * 50)
    logger.info("开始测试: google-videoprism-lvt-base-f16r288 @ %s", BASE_URL)
    logger.info("=" * 50)
