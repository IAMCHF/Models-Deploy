#!/bin/bash
# Retest remaining fixed models after power loss
...
for model in "${FIXED_MODELS[@]}"; do
    ...
    echo "  启动服务..."
    cd "${MODEL_DIR}"
    bash start.sh > /tmp/${model}_retest2.log 2>&1 &
    START_PID=$!
    ...
    echo "  运行 test.py..."
    cd "${MODEL_DIR}/test"
    if python test.py > /tmp/${model}_retest2_result.log 2>&1; then
        echo "  [PASS] 测试通过"
        PASS_LIST+=("$model")
    else
        echo "  [FAIL] test.py 失败"
        echo "  --- 测试输出 ---"
        cat /tmp/${model}_retest2_result.log 2>/dev/null | tail -10
        echo "  --- 服务日志最后10行 ---"
        tail -10 /tmp/${model}_retest2.log 2>/dev/null
        FAIL_LIST+=("$model")
    fi
