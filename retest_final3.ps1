# 重测最后 3 个: docblocklayout(.png后缀修复), koreapeter×2(albumentations已补)
...
Write-Result "========================================"
Write-Result "Retest final 3 - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') (offline, --network none)"
Write-Result "========================================"
foreach ($model in $MODELS.Keys) {
    $maxWait = $MODELS[$model] * 2 + 60
    ...
    if ($ready) {
        ...
        if ($testExit -eq 0) { $passCount++; Write-Host "  [PASS] $model (${elapsed}s)"; Write-Result "[PASS] $model | startup ${elapsed}s | test PASS" }
        else { $failCount++; $failList += $model; Write-Host "  [FAIL] $model"; Write-Result "[FAIL] $model | startup ${elapsed}s | test FAIL (exit=$testExit)" }
...
Write-Result "Retest3 Summary: PASS $passCount / FAIL $failCount / TOTAL $($MODELS.Count)"
if ($failList.Count -gt 0) { Write-Result "Retest3 Failed: $($failList -join ', ')" }
